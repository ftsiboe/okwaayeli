* 100_exhibits.do -- NOT REPAIRED. DO NOT RUN.
*
* Everything below this header is still the DISABILITY study's Stata exhibit
* script, relocated and renamed in the 2026-08 consolidation but not converted.
* It cannot be converted by renaming, for two structural reasons:
*
*   1. It computes GLSS6-vs-GLSS7 TREND differences (`gen Trend=Season-r(min)`,
*      `reg `Var' c.Trend##i.disagCat`, `_b[6bn.Survey]-_b[7.Survey]`).
*      time_poverty is GLSS7 ONLY -- 001 restricts to a single wave. There is no
*      trend to estimate.
...
* this study, via `sheetmodify` -- which errors rather than creating it.
*
* The guard below stops the script. Deleting the guard does not make the script
* correct; deciding what a single-wave, single-indicator exhibit set should
* contain does. See ../AGENT_PROMPT.md steps 4-6.

di as error "100_exhibits.do is unconverted disability code -- see the header."
exit 9



use "$GitHub\ghana\okwaayeli\data-raw\releases\harmonized_data\harmonized_disability_data",clear
...
compress
saveold "$GitHub\ghana\okwaayeli\studies\disability\output\disability_study_data",replace ver(12)


use "$GitHub\ghana\okwaayeli\studies\disability\output\disability_study_data",clear
...
*/ using "$GitHub\ghana\okwaayeli\studies\disability\output\disability_results.xlsx", /*
