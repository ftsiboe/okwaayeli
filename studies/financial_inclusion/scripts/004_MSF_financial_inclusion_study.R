      ...
      tryCatch({ 
        ...
        if(technology_variable %in% "credit_hh" &  
          ...
          for(ddx in c(
            ...
            # SUPPORT ON BOTH ARMS, not just overall support.
            #
            # The mean below is taken with na.rm, so for the loan-module blocks
            # (Source_*, Collateral_*, Use_*) it is the share among BORROWERS:
            # those items are elicited only where a loan exists, and are NA for
            # every non-borrower. Verified 2026-09-02 on the estimation data --
            # Source_1..6, Collateral_1..8 and Use_1..6 each have ZERO
            # non-borrowers with a recorded value.
            #
            # A disaggregation on such a variable has no comparison group: every
            # farmer in scope is treated by construction, so the credit-minus-
            # no-credit gap it yields is not a contrast. That is the defect
            # Appiah-Twumasi raised on v005 Table 6 ("is this even useful given
            # that for some variables all the farmers in that scope will be on
            # one treatment type"), and it is why Table 6's Collateral/trust and
            # Loan-application-processing blocks render blank while
            # .guard_filled() -- which tolerates a table up to 75% empty --
            # lets them ship.
            #
