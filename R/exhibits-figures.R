...
#'
#' @description
#' The shared plot theme: sans text at 9pt, no axis ticks or lines, horizontal
#' grid only, bottom-centred legend. Applied by every `fig_*()` builder so the
#' figures across the studies look like one publication.
#'
#' @return A \pkg{ggplot2} theme object.
#'
#' @family exhibit figures
#' @export
ers_theme <- function() {
  ggplot2::theme(
...
#'
#' @description
#' Reads a set of estimation `.rds` files and binds their summaries into one
#' `data.frame` for the figure builders and the main-specification table.
#'
#' @details
#' Each file is read inside a `tryCatch`, so an unreadable or half-written
#' estimation is skipped rather than aborting the run. If a figure comes back
#' empty, check that every path in `res_list` exists before suspecting the plot.
#'
#' @param res_list Character vector of paths to estimation `.rds` files, normally
#'   built from `study_environment$wd$estimations`.
#' @param study_environment The study environment list from `study_setup()` /
#'   `001_DATA_*.R`.
#'
#' @return A `data.frame` of stacked results.
#'
#' @family exhibit figures
#' @export
...
#'
#' @description
#' Plots the treatment gap across alternative specifications -- functional form,
#' inefficiency distribution, score aggregation, matching algorithm.
#'
#' @param y_title Character. Y-axis title.
#' @param res_list Character vector of paths to estimation `.rds` files.
#' @param colset Character vector of colours.
#' @param study_environment The study environment list.
#'
#' @return A \pkg{ggplot2} object.
...
#' Input and Output Treatment-Effect Figure
#'
#' @description
...
#'
#' @description
#' Plots standardised mean differences and variance ratios across the candidate
#' matching algorithms -- the evidence behind the choice of matched sample.
#' Balance is best where the standardised difference is near zero and the
#' variance ratio near one.
#'
#' @param colset Character vector of colours.
#' @param study_environment The study environment list.
#'
...
#' no study calls it.
#'
#' @param dataFrq `data.frame` of scores to plot.
#' @param colset Character vector of colours.
#' @param study_environment The study environment list.
#'
#' @return A \pkg{ggplot2} object.
...
#' @family exhibit figures
#' @export
fig_distribution <- function(dataFrq, colset = c("orange", "darkgreen"),
                             study_environment) {
...
#' `ag_services`, `disability`, `education`, `financial_inclusion`,
#' `resource_extraction`, `time_poverty`. Delete once none do.
#'
#' @inheritParams fig_distribution
#' @return A \pkg{ggplot2} object.
#'
#' @family exhibit figures
#' @export
fig_dsistribution <- function(dataFrq, colset = c("orange", "darkgreen"),
                              study_environment) {
  warning("fig_dsistribution() is a deprecated misspelling; use fig_distribution().",
          call. = FALSE)
  fig_distribution(dataFrq = dataFrq, colset = colset,
